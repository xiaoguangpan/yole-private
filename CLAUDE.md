...
| 7. B2: Bridge ownership 迁 Rust · [playbook](./docs/refactor/B2-bridge-ownership.md) | ⏳ 待启动（playbook ready） | 3w · M1 RunnerManager scaffold + M2 bridge.ts thin shim + M3 Unix socket / named pipe listener + M4 CLI `session send` / `session watch` + M5 schema 010-011 origin 字段 + M6 agent-api.md 增量 + M7 dogfood 1w 性能 gate。Playbook 2026-05-19 升格完成（[B2-bridge-ownership.md](./docs/refactor/B2-bridge-ownership.md) M1-M7 共 ~75 sub-task，6 个 phase invariant B2-I1~B2-I6）。Cursor → T1.1 |
