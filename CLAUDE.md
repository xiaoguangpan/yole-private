...
| 8. B3: useAppStore 拆 slice + 改订阅 Rust event · [playbook](./docs/refactor/B3-store-slice.md) | 🟡 M1-M4 ✅，M5 待 fresh session | M1-M4 落地（2026-05-19）：**M1 design paperwork**（[slice mapping](./docs/refactor/b3-slice-mapping.md) + [ADR](./docs/refactor/b3-slice-adr.md) + [Rust emit catalogue](./docs/refactor/b3-rust-emit-catalogue.md)）/ **M2 uiStore**（display state，net -61 行）/ **M3 runtimeStore**（M3a LLM concerns + M3b bridge lifecycle，net -1000 LOC from useAppStore，[devlog](./docs/devlog/2026-05-19-b3-m3-complete.md)）/ **M4 sessionsStore + Rust GalleyApi 17 trait methods**（[M4 sub-plan](./docs/refactor/B3-M4-sub-plan.md) `0943c91` + M4a Rust trait + tests `eca7d65` + M4a addendum (set_session_llm) `35f0e78` + M4b 前端抽离 `ad342e9`，[devlog](./docs/devlog/2026-05-19-b3-m4-complete.md)）。M4 单 session 4 commit 推完：useAppStore.ts 2362 → 1431 行 (-931 LOC)，lib/db.ts 883 → 607 行 (-276 LOC，删 persistSession / persistProject / loadSessions / loadProjects / 等 7 functions)，新 sessions.ts 1054 行（**超 B3-I5 600 行硬上限**，M5 同步分子文件 per G11）。frontend write path 100% 走 Rust GalleyApi (`create_session` / `archive_session` / `unarchive_session` / `rename_session` / `set_session_pinned` / `delete_session` / `assign_session_to_project` / `bump_session_after_turn` / `clear_session_unread` / `bulk_archive_sessions` / `bulk_unarchive_sessions` / `bulk_delete_sessions` / `set_session_llm` / `list_projects` / `create_project` / `update_project` / `delete_project` + B1 list_sessions)。沿途穿插：B3 prereq relaxation 把 1 周日历仪式改成事件驱动双层 gate / [perf baseline P1-P4 实测](./docs/refactor/perf-baseline.md) / **4 个 B2 latent bug 修**（CLI 提问 GUI 渲染 `4e7a6e6` / LLM 持久化 `9c36f42` / picker flash `d6a096f` / picker spawning click `317e816`）。Rust 126/126 + TS typecheck + lint + cargo check 全 clean。剩 **M5 messagesStore (~600 行，最复杂 — streaming + approval + ask_user + auto-scroll 互动) + M6 prefsStore (~250 行) + 收尾**。M5 启动门：M4b dogfood 1 天 + M5 sub-plan 写作。最终目标 `useAppStore.ts` ≤ 200 行或彻底删除 |
