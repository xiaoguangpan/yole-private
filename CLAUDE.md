...
| 3. V0.1 七件事 polish | 🔨 进行中 | #1 端到端真跑 ✅（含一波大 polish + 提前做 Markdown / Message actions / 流式生成）；tool_events 审批审计持久化 ✅；Multi-session N-active 核心架构 ✅（含 useShallow 踩坑→store-side enrichment 修复；createSession + activateSession + bumpSessionAfterTurn + Sidebar live status）；剩 Multi-session polish 4 项（title 派生 / summary 写入 / llmDisplayName 同步 / 切换模型按钮接 IPC）+ LRU 5 资源管理（Task 2.5，待 Task 3 配套）+ Session 恢复（Task 3，可参考 launcher `set_state` 协议）+ Settings path picker + Onboarding 真 validation + macOS bundle |
