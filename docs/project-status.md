...
- Git tag / GitHub Release: `v0.0.10` is the current release target; the Windows
...
`v0.0.10` is the current Windows package target. The signed NSIS package
...
1. Confirm reinstall / first launch stays on the silent managed setup path and
   does not show ordinary users the legacy GA onboarding screen.
2. Confirm the Yole points ledger opens from the balance dropdown, stops
   loading on errors, supports retry, and renders real ledger rows from the
   provisioner `/api/account/ledger` endpoint.
...
| Release path | v0.0.10 uses the VPS-hosted Windows download and update endpoints | [release / update SOP](./release-update-sop.md), [demand pool](./demand-pool.md) |
| Windows | v0.0.10 Windows NSIS package is built locally for manual testing | [Windows checklist](./windows-build-checklist.md) |
...
- Current package metadata uses `0.0.10`. For the next release, update:
  - `package.json`
