...
├──────────────────────────────────┤
│ PROJECTS                  [+]    │  始终显示，最多 inline 8 个
│   Folder Galley           pin    │
│   Folder Website                 │
│   查看全部 (N) →                 │  > 8 时打开 ProjectsDialog
├──────────────────────────────────┤
│ Project filter banner            │  仅 activeProjectFilter 时显示
├──────────────────────────────────┤
...
- **Quick Actions 靠顶部**：New Chat / Search 是最高频入口。处于 Project filter 时，新对话继承该 project，并在 label 里显示上下文。
- **PROJECTS 在 timeline 之上**：持续性工作比最近性更稳定；Projects 始终显示，空状态也给一句 muted hint + 新建入口。
- **Project row 不用 emoji**：用 Phosphor `Folder` / `FolderOpen` 表达层级与 filter，避免跨平台 emoji 造成的视觉重量和渲染差异。
- **Project section 不做 inline 展开树**：sidebar 只展示 pinned/recent 的前 8 个 project；超过 8 个用 "查看全部 (N)" 打开 `ProjectsDialog`，那里有搜索。
- **Active project 是 filter + 新对话上下文，不是 Project View 页面**：点击 project 后 sidebar timeline 被过滤，Projects row 高亮，下面出现 filter banner；右侧切到 project-aware EmptyState（placeholder: `在 {Project} 里交代什么？`，第一句话 lazily create 到该 project）。如果当前 session 正在运行 / 等审批 / 等用户回复，只切 filter，不遮掉正在进行的任务。
...
- Inline list：`pinned desc, lastActivityAt desc`，最多 8 个。
- Row：Phosphor `Folder` + name + optional pinned icon。Active filter row 用 `bg-selected`。
- Right-click menu：Pin / Unpin、Rename、Delete。Delete 走 confirm dialog；删除 Project 不删除 session。
- `CreateProjectDialog` 是 420px modal，只收 `name`。rootPath / cwd-binding 已回滚，避免悄悄改变 GA 相对路径语义。
- `ProjectsDialog` 是 640 × 520 modal：搜索 + 全量 project list。不是主区次级页面。
...
- Sidebar 正常显示 Header / Quick Actions / Projects / timeline；没有 session 时只出现一句 muted empty hint。
