...
`Button` 的 `primary` variant 是主 CTA，允许极轻的实体反馈：默认有低位阴影，
hover 微上浮并加深阴影，active 回落。不要把这个效果扩散到 `secondary` /
`ghost`，避免设置页里所有按钮都漂起来。
...
- `测试连接`：发送最小开销真实模型请求，成功后显示延迟；失败时保留 HTTP code，并用人话解释 401 / 403 / 404 / 429 / 网络 / 超时等常见原因。
...
从 Settings 进入时，第一屏保留 `返回设置` 出口；首次安装路径不显示这个出口。
