...
> 本文件以当前两栏 Yole GUI 为准：旧三栏 Inspector、独立 Settings window、Project emoji tree 等历史 spec 已退役。
...
Dark theme 是 Yole light theme 的夜间版本，不是另一个产品方向。视觉目标是
...
- 保留 `serif` 的场景必须是被阅读的 prose 或少量品牌语气：agent answer、
  Markdown headings / blockquote、`Yole` wordmark、About project note。
...
- conversation markdown、用户消息、code block、input / textarea、路径 / key /
  error detail 等内容区域必须保留可选择文本。Yole 是工作台，复制内容是核心任务。
...
- Sidebar **不可折叠**。多 session 是 Yole 的核心产品形态，隐藏 Sidebar 等于隐藏差异化；需要更少 chrome 时通过拖拽缩到 14%。
...
```
┌──────────────────────────────────┐
│ Yole                    ● GA 就绪 │  product name + runtime dot
├──────────────────────────────────┤
│ + 新对话                   ⌘N    │  Quick action
...
│   FolderOpen Yole        +     │  行点击展开/收起；+ 新建项目对话
│     ◐ Session A                  │
│   FolderOpen Website       +     │
...
- **单行 Header**：`Yole` product name + runtime dot 同行。产品名使用 sentence case，不使用全大写 wordmark，避免读成 acronym。`GA 就绪` 只是状态；`GA 未配置` 才可点并进入 Settings → Runtime。
...
- 主题：跟随 Yole 当前主题，light 用 `github-light`，dark 用 `github-dark`
...
- displayName 由 bridge 按 runtime 边界生成：external GA 显示完整 raw name；managed GA 显示 Yole Models 里的显示名或原始 model id（详见 IPC 协议）
...
产品判断：右侧常驻面板让 Yole 读起来像 IDE，而不是本地 agent team orchestrator。把信息放回触发它的上下文，用户少一次"去右边找详情"的认知跳转，也释放了 conversation column 的阅读空间。
...
- 主 CTA：`开始使用 Yole`；只有当前 API Key / Base URL / 模型组合测试成功后可点。
...
UI：嵌入 Health Check Card（详见 §6.1），失败项必须 fix 才能继续，**不允许"以只读模式进入"**（Yole 没 LLM 什么都做不了）。
...
**为什么不直接显示 "401 Unauthorized"**：普通用户看到原始错误不知道下一步。"哪里出错 → 怎么解决"的翻译是 Yole 比裸跑 GA 增值的关键点。
...
- **GA path**：当前路径显示（mono）+ 重选按钮（触发文件夹选择器）
  - 改动后弹 confirm dialog："路径改动需要重启 Yole 才能生效。立即重启？/ 稍后"（不悄悄 kill 所有 session）
...
- 页面分为主视图和维护区：
  - `当前配置模型` 是主视图，显示 Yole 当前会使用的模型队列、默认模型和排序。
  ...
  - 获取模型列表后的模型选择必须使用 Yole 自定义 popover dropdown，不使用浏览器原生 `select`。
...
- App icon + `Yole` 标题（Newsreader medium 18px）
...
- Copy Supervisor SOP：复制 Yole Agent SOP，不写入 GenericAgent memory。
- CLI install / path 指引：帮助可信 Agent 找到 `yole` CLI。
