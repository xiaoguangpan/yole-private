...
- [ ] **T3.9** Dogfood scenario list 跑：
  ...
  - LLM 切换 per-session
  - bridge crash 模拟（kill -9 yole_bridge）→ verify onClose toast + state cleanup
...
- [ ] **T5.11** Dogfood scenarios:
  ...
  - history restore（关 Yole 重开，session active 时 replay history）
