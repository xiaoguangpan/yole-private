...
```
Cursor:   T4b.1  (M4b 前端 sessionsStore 抽离，fresh session 重开)
Status:   🟢 M2 ✅ · M3 ✅ · M4 sub-plan ✅ · M4a ✅ (Rust trait + 47 tests + 16 Tauri commands)
Started:  2026-05-19
Last touch: 2026-05-19 — M4a ship。16 trait method 全部加进 GalleyApi + SqliteGalley impl + Tauri commands + 47 个新测试全过。cargo check / cargo test 123 / pnpm typecheck / pnpm lint 全 clean。Frontend **完全不动**，行为 byte-identical（Tauri 命令注册但 GUI 还没 invoke 它们）— M4b 才接 callers。详 N11 + agent-api.md §8A 新增 16 method 完整 schema。
