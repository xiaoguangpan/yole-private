...
```
Cursor:   T5.1  (M5 messagesStore — fresh session 重开,sub-plan 已 ship)
Status:   🟢 M2 ✅ · M3 ✅ · M4 ✅ (a+addendum+b) · M5 sub-plan ✅
Started:  2026-05-19
Last touch: 2026-05-19 — **M5 sub-plan ship**。[B3-M5-sub-plan.md](./B3-M5-sub-plan.md) 单 markdown commit (0 代码改动)。**关键决策**：single commit M5 (不拆 M5a/M5b)。详 N13。前一 session: M4 完成 (a+addendum+b 三 commit; useAppStore.ts -931 LOC; sessions.ts 1054 行超 600 B3-I5 上限 — M5 同步分子文件预案; lib/db.ts -276 LOC)。
