...
```
Cursor:   v0.2.0-beta.1 beta dogfood / release ceremony; M2 menubar remains optional-gated
Status:   ✅ M1/M3/M4/M5/M6/M7/M8 main path shipped · beta dogfood in progress
Started:  2026-05-20 (paperwork)
Last touch: 2026-05-21 docs routing pass. Settings surface is now Agent, SOP is copy-first, `schemaVersion: 1` is frozen for `v0.2.0-beta.1`, and current state moved to `docs/project-status.md`.
Predecessor: B3 ✅ tag b3-complete
Successor:   v0.2.0-beta.1 milestone ship
...
B4 ship = v0.2.0-beta.1（dual-native orchestrator beta 公开发布）。
...
## Acceptance criteria · B4 算完成（v0.2.0-beta.1）
...
- [ ] **A14**: dogfood 一周（B4 完成后），零 P0 / P1 bug，准备 v0.2 ship — **calendar gate, M9 ship 前必走**
...
- ✅ agent-api.md 顶部 banner 标 "FROZEN for v0.2.0-beta.1" + §1.1 stable identifier sets + §1.2 schema pinning
...
## M9 · B4 acceptance + v0.2 ship 准备 (D65+) · 📋 Paperwork prep complete (2026-05-20)

v0.2.0-beta.1 的 release ceremony。Paperwork prep (T9.0 + T9.3 + T9.4 draft + T9.7) shipped 2026-05-20；剩 dogfood + tag + publish 等 calendar/JC gate。
...
- [ ] **T9.5** Tag v0.2 — `git tag v0.2.0-beta.1` + CI 出 artifact（aarch64.dmg / x64.dmg / x64.exe-setup）
- [ ] **T9.6** GitHub Release publish — draft prerelease；不标 Latest
...
- [ ] **T9.8** 写 B4 完成 devlog + v0.2 release devlog（分两个 entry，B4 是工程过程 / v0.2 是产品 milestone）。**Ship 时写**
