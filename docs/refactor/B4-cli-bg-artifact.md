...
3 件并进，把 Yole 从「dual-native 内部架构 ready」推到「dual-native 对外发布」：

1. **CLI feature-complete**：B1+B2 已实现 6 read 命令 + send/watch；B4 补齐 PRD §11.1 所有 write 命令（session new / btw / stop / archive / restore + project + llm）。每个新命令在 Rust `YoleApi` trait 加 method + Tauri command + CLI subcommand，**单条命令 sub-plan 即可**——M1 比 B3 milestones 更细颗粒。
2. **Background mode (menubar daemon)**：关窗 → 隐藏不退出（**Cmd+Q 才退**）。menubar 图标 + active session badge，下拉菜单 Show/Quit。CLI 才能用——Yole Core 必须 alive 才接 socket 写命令。
3. **Adapter artifacts**：Yole Supervisor SOP for GenericAgent + yole-supervisor skill for Claude + `docs/agent-api.md` v1 publish + discovery file。这些一起 ship 让外部 supervisor（GA bot / Claude / 用户自写 agent）从 day 1 就能用。

B4 shipped as v0.2.0, Yole's first stable dual-native orchestrator release.
...
- **B4-I2**: **localhost only**（[CLAUDE.md](../../CLAUDE.md#2-localhost-only)）。B4 引入的 socket / discovery file / supervisor SOP 任何一条建议「开 TCP / 加 token / 走远程」都被本条款拒绝。Supervisor 远程接入走 GA IM frontend 或 SSH tunnel，不是 Yole 的事。
- **B4-I3**: **数据不离开 Yole**（[CLAUDE.md](../../CLAUDE.md#4-data-stays-in-yole)）。Yole 不存 supervisor ↔ human 的对话内容。Supervisor 通过 CLI 发的 commands + `--reason` 标注存进 Yole（per-session 行动日志），但 supervisor 跟 user 在 IM 里聊的对话**不**存。M7 supervisor 行动日志 GUI 只渲染 Yole-known origin 字段，不接 supervisor 对话回灌。
...
- **B4-I5**: **SOP copy-first**（[Supervisor SOP](../integrations/yole-supervisor-sop.md)）。Settings → Agent 的 "复制 SOP" 按钮只把 `docs/integrations/yole-supervisor-sop.md` 复制到剪贴板；Yole 不写 GenericAgent `memory/`，也不提供 "Install to GA memory" 入口。
- **B4-I6**: **Migration 备份强制**。Schema migration 010-014 在 Yole 内 hard-coded 备份步骤：先 copy `~/Library/Application Support/app.yole/` → `~/Library/Application Support/app.yole.backup.<timestamp>/`，再跑 migration。失败 → 拒启动 + 弹 Finder 到备份目录。dogfood 数据 6+ 月不能丢。
...
- [x] **A3**: Background mode 工作：关窗 → 隐藏，Cmd+Q → 真退。Yole Core 持续跑 (menubar / tray 图标存在) — shipped before v0.2.0
- [x] **A4**: Menubar / tray 图标：点击下拉菜单可 Show Yole / Quit — shipped before v0.2.0
- [x] **A5**: Yole Core 完全退出后 CLI 报 exit 4 "Open Yole first"（沿用 B2 exit 分类）
- [x] **A6**: `~/.config/yole/cli-path`（mac/linux）/ `%APPDATA%\yole\cli-path` (windows) discovery file 在 GUI 首次启动后存在，内容是 CLI binary 绝对路径 — M3 T3.1
- [x] **A7**: Settings → Agent 有 "安装 yole 命令" 按钮，点击触发 sudo + symlink（macOS）；Windows 显示 unsupported copy — M3 T3.3 macOS ship; Win path_install.rs cfg-gated Unsupported
- [x] **A8**: Settings → Agent 有 "复制 SOP" 按钮（[B4-I5](#phase-invariants--b4-特有的硬规则)），把 `yole-supervisor-sop.md` 复制到剪贴板，用户自行发给可信 Agent — M3 T3.4 后本轮改为 copy-first
- [x] **A9**: `docs/integrations/yole-supervisor-sop.md`（GA SOP）写完 — M4 T4.1 ship; JC dogfood 2026-05-20 pass（手动 supervisor flow；IM bot 集成 T4.2-T4.5 留 v0.6+ calendar gate）
- [x] **A10**: `.claude/skills/yole-supervisor/` Claude Skill 包写完 + 在 Claude Code 里加载试用通过 — M5 ship; JC dogfood 2026-05-20 pass
...
- [x] **A13**: 所有 Yole 架构原则（[CLAUDE.md](../../CLAUDE.md)）在 code review 中能逐条 demo：localhost only / CLI 公开契约 / 数据不离开 Yole / 路径 B 不可逆 — [docs/architecture-demo.md](../architecture-demo.md) ship 2026-05-20，4 principle 各自 code refs + grep gates (4 gate 全 exit 0) + tests + devlog provenance
...
- [x] **T1.1** M1.1 prereq commit — `dd4f6cf` (YoleError::RunnerError + socket helpers + tx-aware trait variants + get_pref_json + 6 new tests, 140 total pass)
...
- [ ] ~~**T1.1** `yole session new`~~ — shipped under M1.2 above (atomic via SQLite tx wrap, sub-plan O1)
- [ ] ~~**T1.2** `yole session btw`~~ — shipped under M1.2 above (transient, runner-only per sub-plan §1.5)
- [ ] ~~**T1.3** `yole session stop`~~ — shipped under M1.2 above (Abort 不 Shutdown per sub-plan §1.4)
- [ ] ~~**T1.4** `yole session archive` / `restore`~~ — shipped under M1.2 above (thin wrapper)
- [ ] ~~**T1.5** `yole project create / list / move / archive`~~ — shipped under M1.3 above (rename → `delete` per sub-plan O2; `move` moved to `session move` per O3)
- [ ] ~~**T1.6** `yole llm list / set`~~ — shipped under M1.3 above (list 直读 SQLite prefs cache per sub-plan §1.6)
...
- [ ] **T2.6** Menubar 下拉菜单 —
  - "Open Yole"（重 show window + focus）
  - 状态行 "N active · M idle"（disabled MenuItem, 仅显示）
  - Separator
  - "Quit Yole"（→ `app.exit(0)`）
- [ ] **T2.7** 首次关窗弹一次 toast/dialog「Yole 还在 menubar 跑」+ "Don't show again" pref
...
- [x] **T3.1** Discovery file 写入 — `f0e6306` (2026-05-20). `core/src/discovery.rs` ~280 LOC, writes `~/.config/yole/cli-path` on Mac/Linux + `%APPDATA%\yole\cli-path` on Win; 2 lines (CLI absolute path + `schema_version=1`); idempotent (byte-equal compare before write); non-fatal across 6 outcome branches. 3 lib unit tests.
...
- [x] **T3.3** "Install `yole` to PATH" 按钮 — `d23dfc6` (2026-05-20). macOS via osascript admin-privileges shell + `ln -sf` atomic+idempotent + 4-state PathInstallRow UI (not_installed / installed / other_target / unsupported) + double-layer shell quoting (sh single-quote inside AppleScript string). Windows path stays as M3 follow-up (cfg-gated Unsupported branch + flagged in commit message).
...
## M4 · Yole Supervisor SOP for GenericAgent (D59-D60)
...
- [x] **T4.1** 写 `docs/integrations/yole-supervisor-sop.md` — shipped `bf9e607` (2026-05-20). 434 lines, 9 sections following spec. System-prompt-addendum tone. References agent-api.md as canonical schema.
- [ ] **T4.2** SOP 安装到自己 GA `memory/` (通过 M3 Settings 按钮)
- [ ] **T4.3** Dogfood 1-2 周 — JC 在自己微信 / 飞书 frontend 上跑真 supervisor scenario：「让 GA 帮我开个 session 跑 X 任务」→ GA 读 SOP → 调 CLI → Yole 接收 → GA 反馈结果
...
- SOP 在 `docs/integrations/yole-supervisor-sop.md` 落地 + dogfood 1 周稳定

---

## M5 · Claude `yole-supervisor` Skill (D61)

Claude Code skill package — 让 Claude 用户 install 完直接能调 Yole CLI。

### Sub-tasks

- [x] ~~**T5.0** M5 sub-plan ship~~ — skipped per N15 (4 options pinned via AskUserQuestion; M3-style direct impl chosen)
- [x] **T5.1** 写 `.claude/skills/yole-supervisor/SKILL.md` — 316 行；frontmatter bilingual trigger 关键词 (中英文 IM-style + Claude-Code-style 表达)；body 6 段 (你的角色 + Step 1 Discovery + Step 2 Cheatsheet + Step 3 Scenarios + Step 4 Destructive + Step 5 Origin + Step 6 Exit codes + Self-check)；supervisor identity convention `claude-skill-yole-supervisor/v1` codified
- [x] **T5.2** Auxiliary files — `references/yole-supervisor-sop.md` 448 行 verbatim copy of `docs/integrations/yole-supervisor-sop.md` + sync header (canonical source + last-synced date + drift-resolution rule)；scripts/ **omitted** per user pick (SKILL.md inline bash snippets sufficient — re-add if dogfood reveals Claude regenerating discovery-file resolution repeatedly)
- [x] **T5.3** 在 Claude Code 上 install 这个 skill — symlink `~/.claude/skills/yole-supervisor → /repo/.claude/skills/yole-supervisor` (mirrors lark-* skill convention)；smoke test PASS — harness immediately picked up `yole-supervisor` in the available-skills system reminder with full description visible，frontmatter YAML well-formed，trigger keywords reachable
- [ ] **T5.4** 写一个示例 scenario — "用 Claude 创建 Yole session 跑某 task" + 截图录屏（v0.2 release notes 用）— **defer to JC dogfood + v0.2 release prep**（screen recording needs JC's screen, can't be done headless; pair with M9 release-notes draft）

### M5 完成标志

- A10 **partial tick** — skill files shipped + Claude Code harness loads it (smoke); full A10 requires JC to fire a real trigger prompt (e.g. "帮我看看 Yole 现在跑啥") and confirm Claude reads discovery file → runs CLI → returns sensible output. Dogfood task carried into v0.2 release-prep window alongside T5.4 screenshot scenario.
...
- [x] **T6.4** Schema 修改 vote — **JC approved both pre-freeze fixes via AskUserQuestion** before any code changes. Issue A: `yole version` snake_case → camelCase (one `#[serde(rename_all = "camelCase")]` line on VersionPayload). Issue B: read error envelope `{error, detail:{message}}` → flat `{error, message}` (changed serde annotation from `tag = "error", content = "detail"` to bare `tag = "error"` so enum fields flatten). Plus the missed snake_case `detail.message` fallback in `cli/src/main.rs::main()`'s serde-failure path. Both fixes have zero shipped consumers + zero test breakage (cli/tests grep confirmed no `detail` field parsing anywhere). v1 stays as v1.
...
PRD §6.1 #4 GUI 渲染层 — 让 human user 在 Yole 看到 supervisor 在干啥。
...
- [x] **T7.4** [B4-I3 enforce](#phase-invariants--b4-特有的硬规则) — Honored. Only origin triple persisted in messages table renders. Supervisor↔user IM chat would have to live in Yole DB to render; it doesn't (and won't — see [CLAUDE.md "数据不离开 Yole"](../../CLAUDE.md)). v1 scope explicitly excludes `session archive` / `session move` / `llm set` non-message writes — they don't leave a per-event audit trail in v0.2 data model and won't render here without a new `supervisor_actions` table (punted to v0.6+).
...
- ✅ A12 partial tick — supervisor activity annotation in conversation timeline + TopBar pill both ship + harness clean. Full A12 awaits JC dogfood: fire `yole session send <id> "test" --supervisor=ga-claude-1 --reason="dogfood M7"` against a live session and confirm annotation strip + TopBar pill both render correctly.
...
- [ ] **T9.9** [Open #5 stub](#open-decisions) — 投 Yole Supervisor SOP 到 fudankw.cn/sophub（如果 sophub 接受）。**Post-ship optional**
...
- **G3 (M3 PATH install)** — macOS 的 sudo prompt 走 `osascript with administrator privileges`，但用户拒绝 sudo 怎么办？fallback：toast「请运行：sudo ln -s /Applications/Yole.app/Contents/MacOS/yole /usr/local/bin/yole」给用户手动跑。**不**对 sudo 失败 hard error。
- **G4 (M4 SOP dogfood)** — SOP 写得再清楚 agent 也会误用。第一次 dogfood 重点观察 agent 调 destructive 命令（archive/delete）时是否 confirm。如不 confirm → SOP 加强 + Yole 端 archive 命令是否要 `--confirm-i-mean-it` flag？（倾向**不**加，CLI 走 supervisor 已经是一层隔离）
...
- `README.md` (rewrite) — v0.1 "今天" / "v0.2 之后" 时序分裂整体退役为 v0.2 现在时，v0.1 退化为 blockquote 历史注；功能 today/即将 split 合并为单一 list（含 ⚙️ Yole CLI + 📡 Agent API + 🍱 Background mode + 🔄 backup）；新增 "给 Supervisor Agent 用 · 集成 v0.2" section（GA bot 集成 + Claude Code 集成 + Settings → Agent 复制 SOP 入口）；架构图清 🚧 markers；安装 v0.x.x → v0.2.x；从源码构建 `cd desktop` → `cd gui` + Yole CLI 单独构建段加
...
5. **手写 `copy_dir_all` 14 行** —— `std::fs` 没有，引入 `fs_extra` 只为一处不值。14 行递归足够透明。symlinks 静默跳过 (Yole 数据目录不应有 symlinks)。
6. **失败 dialog 中文 + 同步 blocking_show** —— webview 可能未起，跨进程异步不可靠。用户主要 zh-CN。`std::process::exit(2)` 干净退出。
7. **partial backup 目录留下不清理** —— 避免重启重试无限循环；下次成功新 timestamp 不冲突。release notes 提醒用户可手动清 `app.yole.backup.*`。
...
3. Time Machine 双备份风险 (Yole backup 在 `~/Library/...` 默认 TM 范围内) — release notes 提醒高级用户可 exclude `app.yole.backup.*`
...
**Tests + checks**: 169 cargo tests still passing (M7 doesn't touch Rust); pnpm typecheck clean; pnpm lint clean; cargo check on both core + cli crates clean. Zero existing test breakage. **No new TS unit tests** — the changes are mostly type plumbing + presentational components, and the dogfood smoke test (JC fires a real `yole session send --supervisor=…`) validates the end-to-end behavior more meaningfully than a unit test of `deriveSupervisorActivity` alone would.

**A12 partial tick**: files ship + harness clean. Full A12 needs JC to:
1. Open Yole GUI to warm up a session
2. From a separate terminal: `$(cat ~/.config/yole/cli-path) session send <session-id> "test from supervisor" --supervisor=ga-dogfood --reason="M7 smoke test"`
...
- [ ] **O3** Discovery file 路径在 macOS：`~/.config/yole/`（XDG）vs `~/Library/Application Support/app.yole/`（Apple convention）。**倾向前者**（跨 OS 一致 + supervisor SOP 不用分支）
...
- [ ] **O8 NEW** M3 "Install to PATH" 失败 fallback：silent fallback 提示用户手动跑 / 严格 hard error？**倾向 toast 提示手动命令**（不让 sudo failure 阻塞 Yole 启动）
...
1. Rust 端 YoleApi trait + 实现（如已有 trait method 复用）
...
- 投 Yole Supervisor SOP 到 fudankw.cn/sophub（如果 sophub 接受）
