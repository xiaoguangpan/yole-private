...
**YoleError 加 variant**：`RunnerError { message: String }` → exit 5 + error tag `runner_error`。在 §3.0 prerequisite commit 一并落地。

### 1.4 `session stop` 语义 · Abort vs Shutdown

**Abort** ([ipc.rs:313](../../core/src/ipc.rs))：GA's `agent.abort()` 设 stop_sig + 跳出 run loop；synthesize `run_complete` event with `ABORTED` marker (per [yole_bridge.py:975-980](../../runner/yole_bridge.py))。Bridge **仍活**。下次 `session send` 直接接上，无需 respawn (5-10s startup cost)。
...
M1 沿用：`session.btw` socket handler **不**调 `send_message` trait method。直接 emit `IpcCommand::UserMessage{ text: "/btw <q>" }` 给 runner —— runner [yole_bridge.py:943-948](../../runner/yole_bridge.py) 自动识别 `/btw` 前缀走 `handle_frontend_command` 旁路。
...
**Option B · 走 SQLite prefs**：CLI 直读 `prefs.llm_list` (pref key 是 [hydrate.ts:75](../../gui/src/lib/hydrate.ts) `getPref<LLMOption[]>("llm_list")` 用的同一 key)。延迟 < 50ms；不占 bridge 槽位；不依赖 Yole Core 运行。
...
4. 不需要 Yole Core alive（B4 整 phase 唯一不依赖 Core 的 write-adjacent 命令；inventory 命令已有此 property）
...
3. not found → exit 2 invalid_args "unknown llm '<name>'; try `yole llm list` to see available"
4. cache empty → exit 4 db_unavailable "llm cache empty; open Yole GUI once to warmup"

**Decision**：by-name lookup 而非 by-index。理由：(a) PRD 写「`--llm=...`」自然语言用 name (b) index 是 mykey.py 内部细节不该暴露 supervisor SOP (c) name 在 mykey.py 改顺序后稳定（per [bridge.ts _simplify_llm_name](../../runner/yole_bridge.py) 尊重用户显式 `name`）。
...
fn map_yole_err(request_id: Option<String>, err: YoleError) -> SocketResponse {
    match err {
        YoleError::NotFound { message } => SocketResponse::err(request_id, "not_found", message),
        YoleError::InvalidArgs { message } => SocketResponse::err(request_id, "invalid_args", message),
        YoleError::DbUnavailable { message } => SocketResponse::err(request_id, "db_unavailable", message),
        YoleError::Internal { message } => SocketResponse::err(request_id, "internal", message),
        YoleError::RunnerError { message } => SocketResponse::err(request_id, "runner_error", message),
    }
}
```
...
socket handler:
```rust
let mut tx = yole.pool().begin().await.map_err(/* db_unavailable */)?;
let session = yole.create_session_in_tx(&mut tx, input, origin.clone()).await
    .map_err(|e| { /* tx auto-rollback on drop */ map_yole_err(...) })?;
let msg = yole.send_message_in_tx(&mut tx, session.id.clone(), task, origin).await?;
tx.commit().await.map_err(/* internal */)?;
```

**Option B · socket handler 内 inline SQL (no trait change)**

socket handler 内直 sqlx query call，不走 YoleApi trait。Pro: 0 trait surface change；Con: 复制 200-300 行 SQL/validation 代码，跟既有 `create_session` / `send_message` impl 漂移风险高。
...
1. `SqliteYole` 加 `pool()` accessor return `&SqlitePool` (already exists or trivial getter)
2. `api.rs` trait 加 2 `*_in_tx` method default impl `unimplemented!()` 是反 pattern → 设 trait 强制实现
3. `SqliteYole` impl 把既有 `create_session` body 提取到 helper `do_create_session(executor: impl SqliteExecutor)`，两 method (`create_session` 走 pool / `create_session_in_tx` 走 tx) 都调 helper
...
| **C-C · 4 commit by noun-group** | (1) prereq: YoleError variant + agent-api exit code 5 + origin helper; (2) session-write (new/btw/stop/archive/restore); (3) project + llm; (4) agent-api §5.7-5.17 + integration tests | **推荐**。每 commit 200-400 LOC；可独立 typecheck/cargo check；revert 1 个 noun-group 不影响其它 |
...
4. prereq commit 单独 ship 让 YoleError 加 variant 不混进任何 noun-group commit (mirror B2 M3 SocketResponse 单独 commit pattern)
...
M1.1 prereq commit (fresh session)
  - YoleError::RunnerError variant
  - cli/src/main.rs map_error_tag 加 "runner_error" → RunnerError
  - cli/src/main.rs exit_code_for 加 RunnerError → 5
  - core/src/socket_listener.rs origin_from_args + map_yole_err helper 提取
  - 单 commit "Refactor: B4 M1 prereq — YoleError::RunnerError + socket helpers"
...
**Independence**：M1.1-M1.4 必须 sequential（依赖 YoleError variant）。M2 tray spike 跟整 M1 chain 完全独立 —— spike 不通过不阻塞 M1 ship（spike fail 只阻 M2 实施）。
...
### T1.1 · M1.1 prereq commit · YoleError::RunnerError + helpers + tx-aware trait + PRD §11.1 rename

**Files**:
- `core/src/error.rs`: 加 `RunnerError { message: String }` variant + Display impl + From `RunnerManagerError`-ish if needed
- `cli/src/main.rs`:
  - `exit_code_for(e)` 加 arm `YoleError::RunnerError { .. } => 5`
  - `map_error_tag(tag, msg)` 加 arm `"runner_error" => YoleError::RunnerError { message: msg }`
- `core/src/socket_listener.rs`: 提取
  - `fn origin_from_args(supervisor: Option<String>, reason: Option<String>) -> Origin`
  - `fn map_yole_err(request_id: Option<String>, err: YoleError) -> SocketResponse` （含 RunnerError 新 arm）
  - 把 `dispatch_session_send` body 内 inline error mapping 改用 `map_yole_err`（重构对齐 B2 pattern）
...
- `core/src/db.rs` (or wherever `SqliteYole` impl lives): impl 新 2 trait method + 拆 helper
...
- `docs/PRD.md §11.1` (O2 + O3 resolved): rename `yole project move` → `yole session move <id> --to=<project-id>` + rename `yole project archive` → `yole project delete` + 加 "v0.6+ 再 ship 真 `project archive` (reversible)" 注脚
...
**Commit**: `Refactor: B4 M1 prereq — YoleError::RunnerError + socket helpers + tx-aware trait variants + PRD §11.1 rename (O2+O3)`
...
5. `yole.create_session_in_tx(&mut tx, input, origin)` → SessionBrief（trait 加 `*_in_tx` 变体 OR socket handler 内手 inline SQL；详 §1.9）
6. `yole.send_message_in_tx(&mut tx, brief.id.clone(), task, origin)` → MessageBrief
7. **COMMIT** (`tx.commit().await?`)
8. 任意 step 4-7 失败 → 自动 ROLLBACK + map_yole_err → 适当 exit code (5 runner_error if internal SQL fail; 2/3/4 if validation fail)
...
**Atomicity guarantee (O1 resolved)**: SQLite transaction wrap eliminates partial state。Reject #2 仍 stands —— **不**加新 trait method `create_session_with_first_message` (trait + test 矩阵 cost)；改在 socket handler 内 inline tx 控制 (`pool.begin()` + 复用既有 trait method **如果它们接受 `&mut Transaction` 而非 owned `&self`**)。**Trait 修改成本**：need to add `*_in_tx` 变体 of `create_session` + `send_message` (2 method)，复用既有 body 但接 `&mut Transaction<Sqlite>`. **Verify before T1.2**: SqliteYole impl 当前是否所有 SQL 走 owned pool? 如是，加 in-tx 变体；如已用 connection-borrow 模式，更简单 reuse. 详 §1.9.
...
2. validate session exists：`yole.session_brief(SessionId(session_id))` → not found → exit 3
...
async fn dispatch_session_archive(request_id, args, app, manager) -> SocketResponse {
    ...
    match yole.archive_session(SessionId(parsed.session_id), origin).await {
        Ok(brief) => {
...
3. `yole.assign_session_to_project(SessionId(session_id), to, origin)` → SessionBrief
4. emit Tauri event `session-moved-external` (payload {sessionId, newProjectId})
5. 返 `{session: brief}` exit 0

**Naming rationale (O3 resolved 2026-05-20 PM)**: PRD §11.2 #5 「`yole <noun> <verb>`，noun 是 verb subject」。`move` 的 subject 是 session 不是 project（projects 不动，sessions 在 projects 间移动）。原 PRD §11.1 写「`yole project move`」违反自家 grammar 规则。**修法**：PRD §11.1 同步更新（M1.1 prereq commit 顺便改）+ CLI surface `session move`，agent-api §5.X 不再需要补「subject is session」说明（直接对齐）。

### T1.7 · `project create / list / delete` (O2 resolved, 原 T1.6 4-cmd 改 3-cmd)

3 subcommand: 1 写 (create) + 1 读 (list, no socket) + 1 删 (delete)。

**`project list`**：CLI subcommand 直走 SQLite (no socket)，mirror `sessions list` / `llm list` pattern。CLI subcommand 内开 SqliteYole + 调 `list_projects` trait method。NDJSON output 一行一 ProjectBrief。
...
Command::Llm(LlmCmd::List) => {
    let yole = SqliteYole::open().await?;
    let cached: Option<Vec<Value>> = yole.get_pref("llm_list").await?;
...
Cache shape (from runtime.ts:686 + hydrate.ts:75): `Vec<{index: u32, name: String}>`. M1 内 trait 加 `get_pref<T: DeserializeOwned>(key) -> Result<Option<T>>` method (复用 SqliteYole `prefs` 表查询 — 已在 B3 M6 hydratePrefs 路径用过相同 SQL)。

**Verify**：当前 `SqliteYole` 是否已有 generic prefs read？grep 一下，没有就在 M1.1 prereq commit 加上。
...
3. `yole.set_session_llm(SessionId(session_id), Some(index), Some(name))` → SessionBrief
...
- Setup: tempdir + YOLE_DB_PATH override + spawn socket server in test process
...
```
M1.1 commit: "Refactor: B4 M1 prereq — YoleError::RunnerError + socket helpers + tx-aware trait variants + PRD §11.1 rename"
  → 含 PRD §11.1 改名（O2 project archive→delete + O3 project move→session move）+ YoleError::RunnerError variant + map_yole_err helper + origin_from_args helper + create_session_in_tx / send_message_in_tx trait method
...
| **R8** | **YoleError 加 RunnerError variant 破坏 exhaustive match** — 全 codebase 内 `match YoleError` 凡 exhaustive 的都需要加 arm；漏一处 = TS strict / cargo error | M1.1 prereq commit 内 cargo check 强 enforce；`#[non_exhaustive]` attr **不**加 (内部 crate 不需要 future-proofing；exhaustive match 是 feature) | Low |
...
### V2 · grep gates
- `grep -rn "YoleError::" core/src/ cli/src/ | grep -v test` 所有 exhaustive match 都含 `RunnerError` arm
...
yole version          # → exit 0, JSON
yole llm list         # → exit 0, NDJSON (might be empty)

# Without Core
yole session new "test"  # → exit 4 db_unavailable

# With Core (起 GUI / Tauri dev mode)
yole session new "test from CLI"   # → exit 0, JSON {session, message, dispatch}
yole sessions list                 # → 看到 "test from CLI"
yole session brief <id-from-above> # → SessionBrief
yole session btw <id> "side q"     # → exit 0 or exit 5 (depending bridge alive)
yole session stop <id>             # → exit 0, {dispatch: ...}
yole session archive <id>          # → exit 0; GUI sidebar 该 session 消失
yole session restore <id>          # → exit 0; GUI sidebar 该 session 回来
yole session move <sid> --to=<pid> # → exit 0; GUI 看 session 进 project (O3)
yole session move <sid>            # → exit 0; session 拆出 project (no --to)
yole project create "test proj"    # → exit 0, ProjectBrief
yole project list                  # → NDJSON 含 "test proj"
yole project delete <pid>          # → exit 0; sessions detach to ungrouped (O2)
yole llm set <sid> "glm-4.5-x"     # → exit 0 or exit 2 (cache empty)
```
...
- [ ] `yole session new "fix auth bug"` → GUI sidebar 出现 + first message 已 dispatch（看 conversation 区域）
- [ ] `yole session new "review PR" --project=<pid> --supervisor=ga-claude --reason="user asked"` → session 进 project + origin 三元组在 SQL 内
- [ ] `yole session new "test" --llm=nonexistent-llm` → exit 2 invalid_args **+ verify sessions list 不见此 session** (O1 atomicity invariant — transaction rolled back)
- [ ] `yole session new ""` (empty task) → exit 2 invalid_args + zero side-effect
...
### Cluster 4 · llm ops
- [ ] cold (fresh install) `yole llm list` → empty NDJSON exit 0
- [ ] GUI 起 session warmup → `yole llm list` → 全 LLM 在
...
1. 实现成本 = 在 trait 加 1 method (composite) + SqliteYole impl 内 wrap transaction + 接受 BEGIN/COMMIT 复杂度
...
**理由 (O3 resolved 2026-05-20 PM)**：原 PRD literal 是「project move」，但 PRD §11.2 #5 自家 grammar rule 是「`yole <noun> <verb>` noun 是 subject」。move 的 subject 是 session 不是 project。保留 `project move` = PRD 内部 §11.1 跟 §11.2 矛盾，agent-api §5.14 doc 强补「subject is session」是绕开 grammar rule 而非守住。**rename 比 doc 补丁优**：PRD 是内部文档，纠正 §11.1 比加 §5.14 文档补丁低成本 + 高一致性。
