...
- [x] prototype 的 `BridgeProcess` 设计已被 B1 实施验证（`YoleApi` trait + types 不会大改）
...
- **B2-I5**: socket 路径 **per-user scoped**（macOS/Linux 用 `$TMPDIR/yole-${UID}.sock`，Windows `\\.\pipe\yole-<user>`）。多个 Yole 实例同时跑 → 第二个 instance 启动时检测 socket 已存在 + listener live → exit with informative log。**不**抢 socket，**不**让两个实例 race
...
- [ ] **A3**: CLI `yole session send <id> "<msg>" --supervisor=X --reason=Y` 跑通：从 CLI → Unix socket → runner_manager → bridge stdin → GA → tool dispatch → event 反向流到 GUI 显示
- [ ] **A4**: Yole Core 启动时开 Unix socket listener（macOS/Linux: `$TMPDIR/yole-${UID}.sock`；Windows: `\\.\pipe\yole-<user>`）。`yole` CLI 在 GUI 关闭时调写命令报 `exit 4` + 明确 "Yole Core not running"
- [ ] **A5**: 多 subscriber 正确工作：runner 的 IPC event 同时进入 GUI（Tauri event）+ CLI `watch` 命令的 socket 流 + (optional) future subscribers，互不影响
- [ ] **A6**: runner subprocess lifecycle 正确 — Yole 主进程退出时（正常 / panic / SIGTERM）所有 alive bridges 在 ≤2s 内清理干净，没有 orphan。`pgrep -fl yole_bridge` 在 Yole 退出后 0 命中
...
- [ ] **A9**: Yole GUI dogfood 跑 v0.2 完整 scenario 行为 **0 regression**：multi-session / streaming / ask_user / approval / `/btw` / yolo toggle / set_llm / load_history / pet attach/detach / shutdown clean 全部跑通
...
- [x] **T1.4** 三个错误 enum：`RunnerSpawnError` (PythonNotFound / GaPathInvalid / BridgeCwdInvalid / PathEncoding / SpawnIo / PipeUnavailable) · `SendCommandError` (ProcessGone / Serialize / WriteIo) · `ShutdownError` (NotFound / KillFailed) — hand-rolled 同 YoleError 风格
...
- [x] **T1.15** Integration tests `core/tests/runner_manager_test.rs`：9 tests 真 spawn mock Python subprocess（tempdir 写 mock `runner/yole_bridge.py`）— ready emit / send command 回显 / agent_running toggle / shutdown clean / LRU eviction + active protection / stderr tail / shutdown_all concurrent / respawn replace
...
- [x] **T1.17** `cargo test --workspace` 全过（35 tests · lib 26 + integration 9）；`cargo clippy -p yole-core --all-targets` 对 B2 引入代码 0 warning（B1 残留 2 个 warning 不在 M1 范围 — `src/api/search.rs` derivable_impls + `tests/db_test.rs` too_many_arguments，已用 spawn_task 独立标记）
...
- [ ] **T2.10** dogfood `pnpm tauri dev` (人工 · JC 手动)：
  ...
  - 关 Yole 主窗口 + 等 5s + `pgrep -fl yole_bridge` 0 命中 ✓
  - 触发 approval 拦截 → Card 显示 → approve → tool 跑通 ✓
- [x] **T2.11** CI 加 `cargo test`：[.github/workflows/check.yml](../../.github/workflows/check.yml) 加两步 `cargo test -p yole-core` + `cargo test -p yole-cli`，per-target matrix (macos-15 / windows-latest)。release.yml 不动 — release 不应该跑测试是约定（CI 已经覆盖；release 是 build artifact，跑测试浪费 5 min CI 时间）
...
在 Yole Core 启动早期开 socket / pipe listener。Listener task 接受 connections，每个 connection 是一条 NDJSON 协议（request → response 或 streamed events）。**本 milestone 完成 = Rust 端有了 CLI write 路径的入口，但 CLI 还没用上**（M4 才用）。
...
- [x] **T3.3** `socket_path()`:
  - Unix: `${TMPDIR:-/tmp}/yole-${UID}.sock` (用 hand-rolled `extern "C" fn getuid()` 而非 `libc` / `nix` crate — 6 行代码省一个 dep)
  - Windows: `\\.\pipe\yole-${USERNAME}` (env var `USERNAME` / fallback `USER` / 最终 fallback "unknown")
...
- [ ] **T3.13** Manual smoke (推到 JC dogfood 时跑)：
  - macOS: `nc -U $TMPDIR/yole-$UID.sock` 手动发 `{"command":"ping"}` 看返回
...
- [x] **T4.1** `YoleApi::send_message(session_id, content, origin) -> Result<MessageBrief>` 加进 trait。`MessageBrief` 加 `origin: Option<Origin>` 字段（读路径 None，写路径 Some 回 echo）
- [x] **T4.2** `SqliteYole::send_message` 实现：lookup session 存在 + 非 archived → 计算 next turn_index → 生成 `msg_<ts>_<n>` 形式 id → INSERT messages with origin → UPDATE sessions.last_activity_at + updated_at → 返回 MessageBrief。**不调** RunnerManager (那由 socket transport layer 接着做 — keeps trait decoupled from runner subprocess concerns)
...
- [x] **T4.4** CLI `yole session send <id> "<content>" [--supervisor=X] [--reason=Y]`：clap subcommand，调 `socket_send_recv` → NDJSON 一行 → 解析 `parsed["result"]` 打印 / parsed["error"] → map 到 exit code
- [x] **T4.5** CLI `yole session watch <id>`：hand-roll connection 而不复用 socket_send_recv（保持开放 read loop）→ 流式 NDJSON 行 → server `{"stream":"end"...}` 触发 break → SIGINT 自然关连接（tokio task drop 路径）。**不实现** `--from` / `--tail` backlog (M3 没加 ring buffer，G8 留作 future enhancement)
- [x] **T4.6** `socket_path()` 升级到 `pub` 供 cli crate 使用 — cli 跟 core 共享同一份 path resolution 逻辑，wire-level 不漂
- [x] **T4.7** 错误处理：connect refused → `YoleError::DbUnavailable` → exit 4。server-side typed error (not_found / invalid_args / db_unavailable / internal) 通过 `map_error_tag` 映射到 `YoleError` variant 然后走 `exit_code_for` 同套 exit code 表
...
- [ ] **T4.10** Manual dogfood scenario 1 (人工 · 推到 M7)：启 Yole GUI 开 session A → 终端 `yole session send <A> "hello" --supervisor=jc --reason=test` → GUI A 显示 message + GA reply + SQLite 查 created_via='cli' supervisor='jc'
- [ ] **T4.11** Manual dogfood scenario 2 (watch · 推到 M7)：终端 1 `yole session watch <A>` + 终端 2 `yole session send <A> "test"` → 终端 1 看到 turn_start/turn_end event 系列 + Ctrl-C cleanly exit
...
- [x] **T5.7** `SqliteYole::send_message` 在 INSERT 时 bind `created_via` / `supervisor` / `origin_note` 列 + UPDATE sessions.last_activity_at（保持 sidebar 排序更新）
- [-] **T5.8** Dogfood v0.2 → v0.2 升级路径校验 — pending JC 手动测（M7 段一并跑）
- [x] **T5.9** Tests 覆盖：`SqliteYole::send_message` 路径通过 cli integration test (exit 4 path) 间接验证；happy-path round-trip 留 M7 dogfood + 后续 B3 store-slice test. 当前 8 个 cli integration test pass
...
- [x] **T6.2** §5.5a `yole session send` command 段：args 表 + supervisor / reason 语义 + response shape (message + dispatch 双字段) + fire-and-forget semantics + exit code mapping
- [x] **T6.3** §5.5b `yole session watch` command 段：streaming NDJSON event shape + stream end sentinel + SIGINT exit + 无 backlog (`--from` deferred to future)
...
- **G5 (M3 T3.4)**: Unix socket race detection 用 `try_connect` 判断 stale — 但如果 owner process 在 connect 跟 read 之间挂了，新 instance 看到 EOF 也会判 stale 然后 unlink。**两 instance 高速 start/stop 切换可能造成 socket file 闪烁**。Mitigation: socket 创建后 PID lock file `/tmp/yole-${UID}.pid` 二次校验
...
- **G7 (M4 T4.3)**: O3 决定 fire-and-forget — 但 CLI 端写完 ack 后立刻 `exit 0`，supervisor 看不到 GA 是否真的处理这个 message。Workaround：建议 supervisor 写"send + watch"组合（`yole session send && yole session watch --from=$(...)`）。**Doc 在 agent-api.md M6 T6.2 强调这个 pattern**
...
- **N8 (T1.15 mock subprocess)**: 集成测试用 Python 写 mock runner 而不是真 `runner.yole_bridge`（不依赖 GA 装置）。mock 在 turn_start 后 `time.sleep(0.3)` 给 harness 检查 `agent_running == true` 的窗口 — stdout reader 在 turn_start 后立刻处理下一行 turn_end 会让 flag race 没法 observe。**Gotcha**：写测试时第一版没 sleep，直接 fail；加 sleep 后稳定
...
- **N10 (T1.17 B1 残留 clippy)**: `cargo clippy -p yole-core --all-targets` 抓到 2 个 B1 留下的 warning（`SearchScope::Default` derivable + `seed_message` too_many_arguments）。我没顺手修——按宪法"改动只动必需的"。已 spawn_task 独立标记，等独立 PR
...
- **N40 (chrono_now_iso 手写)**: 写时间戳没引入 `chrono` crate — 用 Howard Hinnant civil-from-days algorithm 手算 9 行。理由：chrono 加 ~30 个 transitive deps + tz database，只为格式化"now()" 不值。Yole 任何场合都用 UTC offset+00:00 (sortable / portable)。**Tradeoff**: 时间字段无法做时区算术，但 Yole 的用法仅是 sort & display origin date — 用不到时区算术
...
- **N46 (T6.5 deferred commands renaming)**: 原 §8 列了 `yole send-message`, `yole archive-session` 等连字符命名 — 但 M4 实际 ship 的是 `yole session send` (subcommand 形式 `<noun> <verb>`)。**§8 全部改 subcommand 命名**与 ship 的一致：`session create / archive / btw`, `project create / rename / delete`, `llm list / switch`

---

## Open decisions

- [ ] **O1** socket 路径在 macOS / Linux / Windows 上的具体格式（[M3 T3.3](#m3--unix-socket--named-pipe-listener-d9-d12)）— **暂行**: macOS/Linux `$TMPDIR/yole-${UID}.sock`；Windows `\\.\pipe\yole-${USERNAME}`。M3 T3.13 dogfood 时确认这两个路径不挡 sandbox / permission
...
- [ ] **O4** RunnerProcess 是 Yole Core 进程 own 还是单独 daemon — **暂行**: Core own。**B4 menubar daemon mode** 让 Core 自己常驻就够，不引入第二个 daemon。如果 B4 实施时发现 Core process 频繁退出（用户关 GUI 主窗口期望"Yole 完全关闭"）影响 socket 可用性，重新评估
...
1. Trait method signature 用 YoleApi 加 async — write 也 async（sqlx async + broadcast async）
