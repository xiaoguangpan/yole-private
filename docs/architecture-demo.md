...
> Yole Core 永远只 listen on AF_UNIX socket / named pipe，不开 TCP，不持有 token。
> 远程访问通过 Supervisor Agent 在外部传输层完成，**不是 Yole 的责任**。

### Code references

- **macOS / Linux**: AF_UNIX socket at `$TMPDIR/yole-$UID.sock` with hand-rolled `getuid` syscall — [`core/src/socket_listener.rs:196-220`](../core/src/socket_listener.rs)
  ```rust
  // SAFETY: getuid is always safe — POSIX guarantees it can't fail.
  let uid = unsafe { libc_getuid() };
  ```
- **Windows**: Named pipe at `\\.\pipe\yole-$USERNAME` — same file (function-level `#[cfg(target_os = "windows")]`)
...
> Yole CLI 的 JSON 输出 schema 是 Yole 对 agent 生态的公开承诺。
...
- **Error enum stable identifiers** — [`core/src/error.rs:12-28`](../core/src/error.rs) `YoleError` 5 variants with stable snake_case tag via serde `#[serde(tag = "error", rename_all = "snake_case")]`
...
> Yole 不存 Supervisor ↔ human 的对话内容。
> Supervisor 通过 CLI 发的命令、命令的 `--reason` 标注存进 Yole（per-session 行动日志），但 supervisor 跟 user 在 IM 里聊的对话不存。
...
- **No Yole → supervisor pull** — Yole CLI / socket has zero command that fetches supervisor-side state (Claude session / GA conversation history / IM messages)
...
- [`docs/agent-api.md`](./agent-api.md) §6A — Origin fields per-via semantics: only what supervisor SENDS to Yole is persisted, never what they receive externally
- [`docs/integrations/yole-supervisor-sop.md`](./integrations/yole-supervisor-sop.md) §6 Origin convention — SOP explicitly tells supervisor bots to pass identity + reason via CLI flags, not push conversation logs

---

## 4. 路径 B 不可逆迁移

> v0.2 起，业务逻辑权威全部在 Rust 端 Yole Core：SQLite 写 / Bridge subprocess ownership / Session 生命周期 / 命令调度。
> 前端（GUI / CLI / 未来扩展）：stateless presenter，订阅 event + invoke 命令。

### Code references — Rust 持有权威

- **SQLite writes via trait** — [`core/src/api.rs`](../core/src/api.rs) `YoleApi` trait with 17 trait methods spanning sessions / messages / projects / origin
- **Trait impl** — [`core/src/db.rs`](../core/src/db.rs) `SqliteYole` is the only writer with `SqlitePool` connection
...
- **CLI uses same trait** — [`cli/src/main.rs`](../cli/src/main.rs) opens `SqliteYole` directly for reads + routes writes through socket (which calls trait); never opens Python subprocess
...
grep -rn "Command::new.*python\|spawn.*yole_bridge" gui/src/ | grep -v "//\|test"
