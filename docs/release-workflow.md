...
> Maintainer-facing document. Use this when preparing or debugging a Yole
> release.

Yole 发版 SOP。本文档定义 v0.2 起的正式发版流程，配合 `.github/workflows/release.yml` 工作。
...
GitHub Actions release.yml 自动触发
       │
       ├─ macos-15 (arm64 runner, native) → Yole_0.2.0-alpha.1_macOS_aarch64.dmg
       ├─ macos-15 (arm64 runner, cross)  → Yole_0.2.0-alpha.1_macOS_x64.dmg   ← v0.1.2 起 CI 出，cross-compile + Rosetta 2
       └─ windows-2022                    → Yole_0.2.0-alpha.1_Windows_x64-setup.exe
       ↓
...
**Mac Intel CI 路径**（v0.1.2 起）：macos-15 arm64 runner + cross-compile + Rosetta 2，详细 trial 验证见 [trial run 26016317898](https://github.com/wangjc683/yole/actions/runs/26016317898)。Rosetta 装载 ~3min，cross-compile 跟 native build 相比多约 2-3min。比保 GitHub macos-13 deprecated runner 更长寿（macos-13 在 2026-27 deprecation 路径上）。本地 build 路径仍然是兜底（CI 不可用 / 紧急 hotfix 时走 [Manual fallback](#manual-fallback-ci-stalled-or-skipped) 方案 B）。
...
- [ ] 如果包含 managed GA runtime / GA baseline / bundled dependency 变更：
      ...
      导入 `managed-ga/code`
- [ ] 如果包含 managed GA runtime 变更：`node scripts/check-managed-ga-payload.mjs` 通过；本地成品包用 `node scripts/check-managed-ga-app-bundle.mjs <Yole.app>` 通过
...
打开 https://github.com/wangjc683/yole/actions 看进度。三个 build job 并行。`release` job 等三个全过才跑。

CI 完成时：GitHub 顶部红圈通知 + 邮件提醒（默认订阅）。

### Step 3. Review draft release

进 https://github.com/wangjc683/yole/releases 看到一个 draft `Yole v0.2.0`：

- **产物列表**：确认 CI 出 3 个文件
  - `Yole_0.2.0_macOS_aarch64.dmg`
  - `Yole_0.2.0_macOS_x64.dmg`
  - `Yole_0.2.0_Windows_x64-setup.exe`
...
自动更新不要直接依赖 GitHub 的 `/releases/latest`。Yole 使用独立分支
`yole-update-channel` 上的静态 manifest：

```text
https://raw.githubusercontent.com/wangjc683/yole/yole-update-channel/updates/stable/latest.json
```

`updates/beta/latest.json` 是旧客户端兼容别名。`YOLE_UPDATER_ENDPOINT` 应该
...
收尾，而是让老版本用户能在 Yole 里升级到新版的正式 release gate。
...
2. 打开 https://github.com/wangjc683/yole/actions/workflows/promote-update-channel.yml
3. 点 **Run workflow**。
...
`yole-update-channel` 分支；同时维护 `updates/beta/latest.json` 作为旧客户端
...
- [ ] 监控 [GitHub Issues](https://github.com/wangjc683/yole/issues) 头 24h，回 bug report
...
1. 打开 https://github.com/wangjc683/yole/actions/workflows/release.yml
...
gh run download <run-id> -n yole-macos-15-aarch64   # 出 Yole_X.Y.Z_macOS_aarch64.dmg
gh run download <run-id> -n yole-windows-2022-x64 # 出 Yole_X.Y.Z_Windows_x64-setup.exe
...
# 产物：core/target/x86_64-apple-darwin/release/bundle/dmg/Yole_X.Y.Z_macOS_x64.dmg

# 3. 起草 GitHub Release notes 到 /tmp/yole-<tag>-notes.md
...
gh release create vX.Y.Z-rc.N \
  --draft --prerelease \
  --title "Yole vX.Y.Z-rc.N · macOS (Release Candidate)" \
  --notes-file /tmp/yole-rc-notes.md \
  Yole_X.Y.Z_macOS_aarch64.dmg \
  Yole_X.Y.Z_macOS_x64.dmg
...
- **本地 build 产物文件名跟 CI 产物一致**（`Yole_X.Y.Z_<arch>.<ext>`），命名一致用户不困惑
...
`macos-13` runner GitHub 已经标 deprecated（具体下线日期 GitHub 没公告，估计 2026 年底-2027 年）。**v0.1.0-alpha.2 起 Yole 已经从 CI matrix 撤掉 macos-13**——比 GitHub 强制下线提前走。

历史走过的兜底路径（按时间序）：

- **v0.1.0-alpha.2 / v0.1.1-alpha.1**: 方案 B —— JC 本地 Intel Mac build + `gh release upload` 手挂
- **v0.1.2 起**: 方案 C —— macos-15 arm64 runner cross-compile x86_64 + Rosetta 2 装载，全 CI 自动出。trial 验证 2026-05-18 [run 26016317898](https://github.com/wangjc683/yole/actions/runs/26016317898) 通过，merge 到 main 作为默认 CI 行为
...
```
Yole.app/Contents/MacOS/desktop:                Mach-O x86_64 ✓
Yole.app/Contents/Resources/python/bin/python3.11:  Mach-O x86_64 ✓
```
...
- `gh release upload v<X.Y.Z> Yole_<X.Y.Z>_macOS_x64.dmg` 挂到 same Release

### 方案 A（drop Intel Mac 支持）

不再考虑——方案 C 跑通后 Intel CI 维护成本可控；Yole 早期用户里 Intel Mac 占比仍可观。

历史记忆：2026-05-15 alpha.2 起为方案 B，2026-05-18 v0.1.1-alpha.1 ship 后验证 + merge 方案 C 成主力。

## Announcement templates

发版同时通常要发通告。每次内容不同，但结构可复用。**这一节是模板，不是文案本身**——具体每次复制改填，发布完通告写到 `/tmp/yole-announce-{zh,en}.md` 用一次就丢，**不入库**。
...
```markdown
🚢 Yole vX.Y just shipped — [one-line product positioning].
...
- `YOLE_UPDATER_PUBKEY`: 嵌入 app 的 Tauri updater public key
- `YOLE_UPDATER_ENDPOINT`: HTTPS updater manifest URL
- `YOLE_PROVISIONER_URL`: HTTPS Yole provisioner URL embedded for first-run
  trial token registration
...
保护逻辑：只要有 session 正在运行，Yole 不会下载 / 安装 / relaunch 更新。
后台检查可以先记住「发现新版本」，等所有任务结束后再继续准备。

```bash
pnpm --dir gui tauri signer generate -w ~/.config/yole/updater.key
```
...
```bash
base64 -D < ~/.config/yole/updater.key.pub
```
...
- GitHub Variables:
  - `YOLE_UPDATER_PUBKEY`: `updater.key.pub` 文件内容
  - `YOLE_UPDATER_ENDPOINT`
  - `YOLE_PROVISIONER_URL`
...
- macOS: `Yole_<version>_macOS_<arch>.app.tar.gz` 和 `.sig`
- Windows: `Yole_<version>_Windows_x64-setup.exe` 和 `.sig`
...
- 不依赖 `/releases/latest/download/latest.json` 作为更新通道；使用上面的显式
  `yole-update-channel` endpoint，避免 draft / prerelease / smoke 前版本泄漏。
