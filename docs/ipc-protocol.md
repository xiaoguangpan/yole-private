...
Yole (Yole) bridge 与 desktop 之间的通信契约。本文件是**唯一真相**——bridge 与 desktop 实现必须以本文件为准；改协议先改本文件。
...
**Stdin/stdout transport** (runner ↔ Yole Core, B2 M1+ Rust-owned):

- **格式**：JSON Lines（每行一个 JSON 对象，UTF-8 编码，以 `\n` 结尾）
- **stdin**：Yole Core → runner 的命令
- **stdout**：runner → Yole Core 的事件
- **stderr**：仅用于 runner 自身崩溃日志，Yole Core 不解析事件（只 rolling buffer 最后 8 行给 abnormal-exit toast）
- **字段命名**：camelCase
- **时间戳**：ISO 8601 带时区（如 `2026-05-07T13:51:00+08:00`）
- **空字符串 vs null**：缺失用 `null` 或省略字段；不要用空字符串当"无"

**Socket transport (B2 M3+)** — for CLI / external agents to reach Yole Core:

- **macOS/Linux**: Unix domain socket at `${TMPDIR:-/tmp}/yole-${UID}.sock`，permission `0600`
- **Windows**: Named pipe at `\\.\pipe\yole-${USERNAME}`，user-scoped namespace
...
Yole Core checks the socket path before binding:
...
- `availableLLMs`：所有可用 LLM 的列表。`name` 是 raw 名字。`displayName` 是 UI 标签：external GA 保留完整 raw name；managed GA 使用 Yole 模型配置里的显示名，未设置时使用原始 model id。
- session 持久化不要只存 `index`：external GA 使用 `name` 作为稳定身份；managed GA 使用 Yole `managed_models.id`，再在启动时解析到当前 index。
...
- `source`：GA 的 source 字段（`"yole"` / `"system"` 等）
...
- `category`：`"bridge" | "runtime" | "business"` —— 决定 desktop 显示位置
  ...
  - `"business"`：Yole 业务错误（attach 路径非法、SQLite 损坏、历史恢复失败）→ desktop 渲染为 toast
...
**hint 的产品意义**：普通用户看到原始错误（"401 Unauthorized"）不知道下一步。bridge 把"哪里出错"翻译成"怎么解决"是 Yole 比裸跑 GA 增值的关键点。
...
`attach_pet` 命令成功后发出。Bridge 已 spawn `<ga_path>/frontends/desktop_pet_v2.pyw` 子进程并在该 session 的 agent 上注册了 `_turn_end_hooks['yole_pet_{sessionId}']`。
...
bridge 收到后更新 `SessionState.yolo_mode`。下一个 tool dispatch 时 `YoleHandler.needs_approval` 第一行检查此 flag——为真则直接放行（不 emit `tool_call_pending`，仍 emit `tool_call_start` / `tool_call_end`）。
...
3. 注册 `agent._turn_end_hooks[f"yole_pet_{sessionId}"]`——每个 turn_end 用 `urllib.request.urlopen` POST 进展到 `http://127.0.0.1:{port}/?msg=...`
...
- [x] **`load_history` messages 数据结构** — 已 e2e 验证：`NativeClaudeSession` 的 `backend.history` 是 `[{role, content: [{type:"text", text:str}, ...]}]`（Anthropic native messages 格式）。`runner/yole_bridge.py:_load_history` 把 GUI 传来的简单 string content 适配为 native blocks。**未验证**：`NativeOAISession` / `ClaudeSession` / `LLMSession` / `MixinSession` 的 history 形态可能不同，需要对应 adapter。当前 V0.1 只在 `NativeClaudeSession` 下保证恢复语义。
...
- [x] **`error` 事件结构化字段** — `category` / `severity` / `retryable` / `hint` 四字段在 v0.1 落地（见 §4.10）。runner 端 LLM 调用错误的 hint 推断逻辑见 `runner/yole_bridge.py` 的 `_classify_error`。
