...
- **Step 0 欢迎页**：大标题 `GenericAgent Workbench` + 衬线副标题 + 三件事简列（多 session / 审批 / 历史恢复）+ charcoal `开始` CTA + footer 一行 muted 文案"不会修改你的 GA，删除 Workbench 后 GA 独立可用"
...
- **Step 2 Health Check**：跑 5 项检查（路径 / Python / agentmain import / mykey.py 存在 / 至少一个 LLM 配置可解析）。**全过才能继续**——Workbench 没 LLM 什么都做不了，不允许"以只读模式进入"
...
- **Empty state 大标题 "Welcome to GA Workbench"**：onboarding 已经说过，empty state 应该激发输入而不是再次 welcome
...
- **Health Check 失败时允许"以只读模式进入"**：Workbench 没 LLM 什么都做不了，应该一步到位 fix
