...
using Yole" failed with:

```text
opening C:\Users\QW\AppData\Roaming\app.yole\data\yole.db:
...
- The source of truth for `sqlite:yole.db` is `tauri-plugin-sql`. It opens
  the DB under Tauri's app config directory: `%APPDATA%/app.yole/yole.db`
  on Windows.
...
- The pre-migration backup gate now probes the same directory as the migration
  ...
  migrated.
- `YOLE_DB_PATH` remains the test and advanced override path.
...
- **Only create `%APPDATA%/app.yole/data`**: that would hide the immediate
