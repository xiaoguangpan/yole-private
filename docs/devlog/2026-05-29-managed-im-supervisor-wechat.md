...
`v0.2.0-alpha.2` 的核心新增面向内置 GA 用户：把 IM 接入做成 Yole 托管的 Managed IM Supervisor。第一阶段只开放微信，目标是让普通用户在 Settings 里点接入、扫码，然后只要 Yole 运行，就能从微信里给 Yole 发消息。

这次讨论里同时对齐了一个边界：外部 GA 用户不应该被 Yole 的托管 IM / 模型配置打扰。外部 GA 本来就由高级用户自己管理环境，所以 Settings -> IM 在 external runtime 下隐藏；已经接入的托管微信进程不因短暂切换 external 而自动停止。
...
- 微信接入复用 GA 官方 iLink 扫码流程，不加绑定码、allowlist 或 Yole 自定义授权层。
- Yole Core 启动 `runner.managed_im_supervisor`，注入 managed model loader、managed state root、runtime prompt、persona prompt 和紧凑 IM Supervisor prompt。
- 不把完整 Supervisor SOP 常驻进 system prompt。Yole materialize 一份自己管理的 SOP reference 文件，IM Agent 在复杂调度或规则不确定时自行读取。
- 微信 token、二维码和日志放在 Yole managed state 下的 `im/wechat/*`，不污染官方默认 `~/.wxbot/token.json`。
...
- **绑定码 / allowlist**：安全感来自 Yole 自己的流程，但会偏离 GA 官方微信接入方式，并增加普通用户理解成本。
...
- **外部 GA 模式仍显示 Settings -> IM**：容易让用户误以为 Yole 会管理外部 GA 的 IM 接入，违背 external runtime 边界。
