...
Yole 的应用内检查更新和后台准备更新已经接上 Tauri updater，但真实用户能不能
更新取决于另一个更脆的边界：live beta manifest 是否已经发布到
`yole-update-channel`。这次检查发现 GitHub secrets / variables 已配置，但
...
- `promote-update-channel.yml` 推送 `yole-update-channel` 后自动运行 verifier。
...
`/d/a/yole/yole/managed-ga/code` into Windows Python's `sys.path`, so
...
- `yole-macos-15-x64`
- `yole-macos-15-aarch64`
- `yole-windows-2022-x64`
