...
>
> **v0.3 核心变化**：产品定位从「GA 的本地桌面工作台」reframe 为「**本地 agent team orchestrator，dual-native for human and agent**」。新增 Yole CLI 一等公民地位、Supervisor Agent 概念、localhost-only 架构原则、CLI 公开契约面（agent-api）。详细决策叙事见 [2026-05-15 vision pivot devlog](./devlog/2026-05-15-vision-pivot-to-orchestrator.md)。
>
> **本文件与 DESIGN.md 的关系**：DESIGN.md 继续负责 GUI 视觉与交互设计规则。CLI 设计契约见独立的 [docs/agent-api.md](./agent-api.md)（v0.2 ship 时 publish）。

## 1. 一句话定位

**Yole** is a **local agent team orchestrator, native for both human and agent**.

中文：**本地 agent team 编排器，人和 agent 都是一等公民**。

它有两个对等的前端：

- **Yole GUI**——给坐在桌前的 human operator 看进度、写指令、审批
- **Yole CLI**——给 **Supervisor Agent**（外部 agent，可能是另一个 GA、可能是 Claude，可能是用户自己写的）远程操作整个 session team

两个前端共享同一个 **Yole Core**（Rust 端权威层），数据 / 状态 / 命令调度都从这里走。
...
v0.1 Yole 解决了第 1 个（multi-session 桌面 GUI），第 2 / 3 个是 v0.2 通过 dual-native 架构解决：

- **桌面**：human operator 用 Yole GUI 像 v0.1 一样管 session team
- **远端**：human 通过 IM 跟 Supervisor Agent 对话，Supervisor Agent 通过 localhost CLI 控制 Yole，所有 session 落到同一个 Yole 数据库
- **回桌面**：所有外面派的任务都在 Yole GUI 里一目了然，可以手动接管继续

Yole 自身永远是本地应用，远程传输不是 Yole 的责任——那部分由 Supervisor Agent 的外部传输层（如 GA 自己的 IM frontend）负责。
...
- 修改 agent runtime 源码（v0.2 = GA：`~/Documents/GenericAgent/` 下任何文件）
- 修改 runtime memory / 配置文件
- 接管 runtime 运行环境（不动 venv、不改 PATH）
- 自动升级 runtime

Yole **可以**做（即「扩展式 attach」）：
...
**Supervisor SOP 交付**：用户在 Yole Settings 内主动点击 "复制 SOP" 按钮，Yole 只把 SOP 文本放进剪贴板。用户自己决定发给哪个 Agent；Yole 不写 GA `memory/`。

**保证**：用户随时可以删除 Yole，GA 独立运行不受影响。

### 4.2 Localhost only

**Yole Core 永远 only listen on AF_UNIX / named pipe，不开 TCP，不持有 token。**
...
Yole CLI 输出的 JSON schema 是 Yole 对 agent 生态的公开承诺：

- 详细规范见 [docs/agent-api.md](./agent-api.md)（v0.2 ship 时 publish）
...
- 现有 GA 重度用户（v0.1 已经在用 Yole 的人，自然过渡）
...
继承 v0.1 七件事的成果，v0.2 新增 5 件：

1. **Yole Core**：Rust 端权威层。bridge 管理 / SQLite 写 / 命令调度 / event broadcast 都在 Rust 完成。前端（GUI / CLI / 未来 web / mobile）是 stateless presenter
2. **Yole CLI v1**：13+ 个子命令覆盖 inventory / 操作 / 项目 / 配置；agent-first 输出（NDJSON / JSON 默认）；schema versioned；公开 [agent-api.md](./agent-api.md) 契约
3. **Background mode**：menubar daemon 形态。关窗 → 隐藏不退出。Yole Core 没在跑 = CLI 全部命令报错 "Open Yole first"
4. **Supervisor 行动日志 per-session**：每个 session 的 timeline 里穿插显示 human / supervisor 对此 session 的动作 + reason，回桌面一眼看懂"昨晚干了啥"
5. **Yole Supervisor SOP for GA** + **yole-supervisor skill** for Claude：两个 adapter artifact 随 v0.2 发布，让用户能立即在 GA 或 Claude 上把 Yole 当作 tool 使用

继承自 v0.1（v0.2 必须仍然成立）：
...
- **Yole 作 chat platform**：不存 Supervisor ↔ human 对话，只存动作 + reason
...
| 外部 agent 通过 CLI 驱动 Yole | **Supervisor Agent** | 大写 S 首字母正式；小写日常 |
| 坐桌前用 GUI 的人 | **human operator** | 跟 Supervisor 配对 |
| Supervisor 远程操作 Yole 这件事 | **agent-driven operation** | 不用 "CEO mode" / "remote control" |
| Rust 端权威层 | **Yole Core** | 专有名词 |
| GUI 前端 | **Yole GUI** | 桌面 React app |
| CLI 前端 | **Yole CLI** | 命令行 binary `yole` |
...
- **Settings → Agent**：可选 CLI shortcut、Copy Agent SOP、Agent API 文档入口

## 9. 数据模型

继承 v0.2 的 Session / Project / Tool Event / ConversationMessage。v0.2 新增字段：
...
- 没有 `supervisor_registry` 表（permission system 雏形，v0.2 拒）
- 没有 supervisor ↔ human 对话存储表（不做 chat platform）

## 10. Yole Core（Rust 端权威层）

### 10.1 职责

| 职责 | v0.1 在哪 | v0.2 在哪 |
|---|---|---|
| SQLite 读写 | TypeScript (`gui/src/lib/db.ts` 778 行) | Rust (Yole Core)，TypeScript 改为通过 invoke 调用 |
| Bridge 子进程 spawn / 管理 | TypeScript (`gui/src/lib/bridge.ts` 203 行) | Rust (Yole Core) 持有 child handle |
| IPC event 分发 | TypeScript (`gui/src/lib/ipc-handlers.ts` 1011 行) | Rust (Yole Core) emit event 到所有 subscriber |
| 命令调度 | TypeScript (Zustand store actions, 2727 行 store 一部分) | Rust (Yole Core) 暴露 trait |
| 状态管理 (authority) | TypeScript (Zustand store) | Rust (Yole Core) 内部 state，前端订阅 |
| UI state (selected session, modals open, etc.) | TypeScript (Zustand store) | TypeScript (拆 slice 后保留) |

### 10.2 Core API（trait + 多 transport）

**single source of truth**：Rust 用一个 trait 定义 "Yole API surface"：
...
yole session brief <id>                # digested 1-2 行
yole session show <id> [--tail=N]      # 完整 message log
yole status                            # 一句话 team 总览
yole health                            # 类似 GUI health check 5 项
...
yole session btw <id> "<q>" [--supervisor=...]
yole session stop <id>
yole session archive <id>
yole session restore <id>
yole session move <id> [--to=<project-id>] [--supervisor=...]   # session 是 subject；no --to = 拆出 project
yole session watch <id> [--filter=...] [--until=idle]            # NDJSON stream

# Project
yole project create "<name>" [--description=...] [--supervisor=...]
yole project list
yole project delete <id> [--supervisor=...] [--reason=...]   # v0.2: destructive，sessions 自动拆到 ungrouped；v0.6+ 再 ship 真正的 `project archive` (reversible)
...
5. **Command grammar**：`yole <noun> <verb>`（`session create` 不是 `create-session`）
6. **`--pretty` 是 derived view**：JSON canonical

### 11.3 Supervisor identity 字段

`--supervisor=<freeform-string>` 由 supervisor 自己定（如 `ga-wechat-bot`、`claude-skill-yole-mgr/v1.2`）。Yole 只记录、不校验、不注册。

### 11.4 不在 v0.2 surface 的命令

- `yole pet attach / detach`（桌宠，supervisor 用不到）
- `yole config get/set`（Settings 是 GUI 的事）
- `yole memory ...`（GA memory 由 GA 自己管，宪法约束）
- `yole supervisors register / list`（permission system 雏形，v0.6+ 候选）
...
```
macOS:   /Applications/Yole.app/Contents/MacOS/yole
Windows: C:\Program Files\Yole\yole.exe
```
...
Settings → Agent → "安装 yole 命令" 按钮：

- macOS：弹 sudo 创建 `/usr/local/bin/yole` → CLI 绝对路径 symlink
- Windows：写用户级 PATH（不需 admin）

### 12.4 v0.6+ 候选

- Homebrew tap `brew install yole`
...
- Cmd+Q 或 menubar "Quit Yole" 才真退
- Yole Core 完全退出 = CLI 全部命令报错 "Open Yole first"
...
- "Launch at login" toggle (default OFF for v0.2)
- "Quit Yole"

### 13.4 首启引导

第一次关窗时弹一次 "Yole 还在 menubar 跑哦，要彻底退按 Cmd+Q" 的引导（写到 `prefs.background_mode_intro_seen`），之后不打扰。

## 14. Adapter artifacts（随 v0.2 发布）

### 14.1 Yole Supervisor SOP for Agents

文件：`docs/integrations/yole-supervisor-sop.md`（仓库内）+ 可投到 fudankw.cn/sophub
...
**交付方式**：Yole Settings 提供"复制 SOP"按钮，把 SOP 文本复制到剪贴板。用户自己决定发给哪个 Supervisor Agent。Yole 不写 GenericAgent `memory/`，避免把 Integration 入口绑定到单一 agent runtime。

### 14.2 yole-supervisor skill for Claude

文件：`.claude/skills/yole-supervisor/`（仓库内 + 可让 JC 发布到 Claude skill marketplace）
...
- 稳定性承诺（schemaVersion 1 内 additive-only）
- 错误码 enum + 解释
...
继承 v0.1 onboarding flow（welcome / attach / health check / new chat），v0.2 新增引导：

1. v0.1 → v0.2 升级用户首次启动：弹"v0.2 新增 Yole CLI"提示，引导到 Settings → Agent
2. 全新用户首次启动后：Settings 一个低调 banner "想让 supervisor agent 远程控制 Yole？看 [docs/agent-api.md]"
...
`app.yole` 保持不动。v0.1 用户的数据目录 `~/Library/Application Support/app.yole/` 直接被 v0.2 接管，所有 sessions / projects / messages 平滑过渡。
...
**风险**：v0.2 schema 设错 → v0.6+ 想加字段不 backward compatible → 用户 SOP 全断。
...
- 公开 [agent-api.md](./agent-api.md) 在 v0.2 ship 时定稿

### 18.3 Yole Core ↔ Tauri WebView lifecycle

**风险**：路径 B 下 Yole Core 是 Rust，权威跟 WebView 解耦。但 Tauri 进程 lifecycle 跟主窗口绑定——"close window 不退出 process" 是否可靠？menubar daemon mode 下 WebView 是否 destroyed？

**缓解**：prototype 阶段 + B4 background mode 实施时专门验证 + dogfood。

### 18.4 v0.1 → v0.2 schema migration
...
- v0.2 release 前 JC 自己 dogfood 跑过 migration
- release notes 显式说明 migration 行为

### 18.5 Supervisor 误用 destructive 命令

**风险**：Supervisor agent 听信 user 的"删了它"未经 confirm 就跑 `yole session archive` / `yole project archive`。
...
| 前端 | Yole GUI + Yole CLI 对等 |
| 权威层 | Yole Core (Rust) |
| Agent runtime | GA only (v0.2)，架构允许将来扩展 |
...
| 权威层 | TypeScript (React) | Rust (Yole Core) |
...
| Stage 范围 | 3.7 收尾 | B1-B4 重构到 v0.2 |
