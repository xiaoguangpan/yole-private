<!--
This file is a verbatim copy of docs/integrations/yole-supervisor-sop.md
shipped inside the `yole-supervisor` Claude Skill so the skill stays
...
| `"$YOLE" project follow <id> --tail=10 --until-idle --final-show` | Follow a Project batch until all child sessions are idle, then emit final context |
...
batch history in Yole. Archiving sessions or deleting the Project requires
confirmation.
